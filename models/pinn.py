...
class JointPoissonPINN(nn.Module):
    """
    Sieć neuronowa PINN (MLP) rozwiązująca jednocześnie wektorowe równanie Laplace'a dla pola u
    oraz równanie Poissona dla funkcji bezpieczeństwa h na podstawie mapy zajętości.
    Wejście: (x, y) -> Wyjście: (u_x, u_y, h)
    Przeznaczona do uczenia punktowego (ciągłego) pojedynczej mapy.
    """

    def __init__(self):
        super(JointPoissonPINN, self).__init__()
        # Głęboka sieć MLP przystosowana do reprezentacji gładkich pól fizycznych w R^2
        self.net = nn.Sequential(
            ...
            nn.Linear(128, 128),
            nn.Tanh(),
            nn.Linear(128, 3),  # Wyjścia: u_x, u_y, h
        )
...
class UNetJointPoisson(nn.Module):
    """
    Model UNet przyjmujący siatkę zajętości [B, 1, H, W] i zwracający 3 kanały [B, 3, H, W]:
    - Kanał 0: Pole u_x
    - Kanał 1: Pole u_y
    - Kanał 2: Pole bezpieczeństwa h
    Przeznaczona do uczenia splotowego (obrazowego) na bazie wielu map.
    """

    def __init__(self, in_channels=1, out_channels=3):
        super(UNetJointPoisson, self).__init__()
...
def calc_poisson_pinn_loss(pred_3ch, grid, dx=10.0 / 512.0):
    """
    Oblicza fizyczny błąd PDE (Physics-Informed Loss) dla całej paczki danych przy użyciu splotów 2D.
    pred_3ch: [B, 3, H, W] -> Kanały: u_x, u_y, h
    grid: [B, 1, H, W] -> Siatka zajętości (0: wolna przestrzeń, 1: przeszkoda)
    """
    ...
    # Norma gradientu pola pomocniczego u
...
def generate_pinn_data_from_grid(grid):
    """
    Ekstrahuje punkty treningowe bezpośrednio z wczytanej siatki zajętości o dowolnej rozdzielczości.
    Używane głównie przy uczeniu ciągłym MLP (punktowym).
    """
...
def compute_joint_pde_residuals(model, x_pde):
    """
    Oblicza błędy residualne (Laplace'a i Poissona) przy użyciu Autogradu w jednym kroku.
    Używane w klasycznym ciągłym solverze PINN MLP.
    """
    out = model(x_pde)
    u_x = out[:, 0:1]
    u_y = out[:, 1:2]
    ...
    # Pierwsze pochodne u_x oraz u_y
    ...
    # Pierwsze pochodne funkcji h
    grad_h = torch.autograd.grad(
        h, x_pde, grad_outputs=torch.ones_like(h), create_graph=True
    )[0]
    dh_dx = grad_h[:, 0:1]
    dh_dy = grad_h[:, 1:2]

    # Drugie pochodne dla Laplace'a u_x i u_y
    ...
    # Drugie pochodne dla Poissona h
    ...
    # ||grad u|| = sqrt( (dux/dx)^2 + (dux/dy)^2 + (duy/dx)^2 + (duy/dy)^2 )
    norm_grad_u = torch.sqrt(
        dux_dx**2 + dux_dy**2 + duy_dx**2 + duy_dy**2 + 1e-8
    )

    pde_res_ux = laplacian_ux
    pde_res_uy = laplacian_uy
    pde_res_h = lap_h + norm_grad_u

    return pde_res_ux, pde_res_uy, pde_res_h


def predict_safety_with_gradients(model, x, y):
    """
    Funkcja pomocnicza dla planisty RRT*. Pozwala na odpytanie modelu o wartość h
    oraz jej analityczne gradienty w dowolnym punkcie przestrzeni ciągłej.
    """
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        out_bc = model(X_bc)
        U_bc_pred = out_bc[:, 0:2]
        h_bc_pred = out_bc[:, 2:3]

        loss_bc_u = torch.mean((U_bc_pred - U_bc) ** 2)
        loss_bc_h_obstacles = torch.mean(h_bc_pred**2)

        out_outer = model(X_outer)
        h_outer_pred = out_outer[:, 2:3]
